    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function renderDashboard() {
            ...
            plots.forEach((plot, index) => {
                ...
                harvestDate.setDate(harvestDate.getDate() + cData.duree_jours);
        ...
        <div class="mt-1 flex items-center gap-2">
            ...
            <select onchange="window.changerSante(${index}, this.value)" class="text-xs rounded border border-gray-200 p-1 bg-white outline-none ${plot.sante === 'Malade' ? 'text-red-600 font-bold' : (plot.sante === 'Stress' ? 'text-orange-500 font-bold' : 'text-green-600 font-bold')}">
                ...
                // Generate Tasks from Etapes (approximate weeks to days)
                if (cData.etapes) {
cData.etapes.forEach(etape => {
                    // Very simple parsing of "Semaine X" to days
            ...
            const now = new Date();
            const nextTasks = allTasks.filter(t => t.date >= new Date(now.setDate(now.getDate() - 7))); // Last 7 days and future

            if(nextTasks.length === 0) {
                remindersList.innerHTML = '<p class="text-gray-500">Pas de tâches imminentes.</p>';
            } else {
                nextTasks.slice(0, 5).forEach(task => { // Show max 5
                    ...
                    remindersList.innerHTML += `
                        <div class="bg-white p-3 rounded shadow-sm border-l-4 border-yellow-500 flex justify-between items-center">
                            <div>
